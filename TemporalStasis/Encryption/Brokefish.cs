...
public class Brokefish {
    ...
    public Brokefish(IEnumerable<byte> key) {
        foreach (var (i, keyFragment) in this.WrappingUInt32(key, this.p.Length))
            this.p[i] ^= keyFragment;

        uint l = 0, r = 0;
        for (var i = 0; i < this.p.Length; i += 2)
            (l, r) = (this.p[i], this.p[i + 1]) = this.Encrypt(l, r);

        for (var i = 0; i < this.s.GetLength(0); i++)
        for (var j = 0; j < this.s.GetLength(1); j += 2)
            (l, r) = (this.s[i, j], this.s[i, j + 1]) = this.Encrypt(l, r);
    }
    ...
    public byte[] Encrypt(byte[] data) {
        ...
        for (int i = 0; i < paddedLength; i += 8) {
            var (l, r) = this.Encrypt(BitConverter.ToUInt32(buffer, i), BitConverter.ToUInt32(buffer, i + 4));
    ...
    public void Decrypt(ref byte[] data) {
        for (int i = 0; i < data.Length; i += 8) {
            var (l, r) = this.Decrypt(BitConverter.ToUInt32(data, i), BitConverter.ToUInt32(data, i + 4));
